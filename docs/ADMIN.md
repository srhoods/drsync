...
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine, and it happens on **every** agent, not one host | Was root-caused to the agent's cross-pool work-stealing (default on) starving its own copy queue's drain side under contention — a stolen walk shard's files re-enter the copy engine from the stealing thread itself, and only 1 copy thread was reserved as a guaranteed drainer regardless of pool size. Fixed (`agent/src/poolsize.c`) by scaling the reserve to 25% of `-C`; see `docs/DESIGN-agent.md` §3.1 for the full mechanism. If you still see this on an up-to-date agent build, or the previous row below (single-host / systemic) doesn't explain it, it's a new instance of the same class of bug — check whether `-S` (stealing disabled) makes it go away, same diagnostic fleet A/B this one was root-caused with. |
