...
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine, especially at high `-w` (walker) thread counts | **Ongoing investigation** (`docs/DESIGN-agent.md` §3.1–§3.6) — several real bugs have been found and fixed along the way (copy-pool reserve scaling, a dedicated writer thread, a heartbeat priority mailbox, an O(n) lease-table scan) but none has fully resolved the reported rate, and `spread_mode: off` / lower shard counts made no material difference either. As of the last update: every timing signal on both sides checks out correctly (heartbeats queued/sent/received on schedule with matching sequence numbers and sane held-counts; no slow coordinator dispatch; no store-lock contention) — so the current diagnostics trace by lease **identity** instead: does the specific lease that expires actually appear in the `held_lease_ids` a heartbeat reported just before it did? Full id lists are now logged on both sides (`"heartbeat queued"`/`"heartbeat received"` with `ids=[...]`/`held_ids`), a `"heartbeat renewal did not match every held lease"` warning fires on any gap, and `"lease expired"` now carries the `lease_id` itself so one specific expiry can be grepped through both logs. If you're chasing this, that's the trace to run: pull one `lease_id` from a `"lease expired"` line and search both agent and coordinator logs for it. |
