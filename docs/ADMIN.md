...
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine, and it happens on **every** agent, not one host, especially at high `-w`/`-C` thread counts | Confirmed with `ss -tn` (agent control-connection `Send-Q` non-zero at the exact moment leases expired): outbound traffic volume (results, journal batches — scales with `-w`/`-C`) can back up the agent's TCP send buffer badly enough to delay the heartbeat behind it. Two fixes landed for this, in order — see `docs/DESIGN-agent.md` §3.1–§3.3 for the full history, including why the first two were each insufficient on their own: §3.1 (copy-pool reserve scaling, targeted work-stealing — not the cause), §3.2 (dedicated writer thread, so a full send buffer can't block the heartbeat *timer* thread — insufficient alone: a heartbeat could still queue behind a large batch of already-queued traffic), §3.3 (a priority mailbox so a heartbeat always jumps the queue ahead of bulk traffic, checked between every individual message the writer sends, not just once per wakeup). If the requeue rate persists on a build with all three: the next candidate is coordinator-side, not agent-side — the coordinator's single store-write mutex can stall one agent's connection behind another's dispatch; see §3.3's closing note. To check: `ss -tn \| grep :<agent-port>` on an agent during a run, watching `Send-Q` on the coordinator connection. |
