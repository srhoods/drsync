...
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine, and it happens on **every** agent, not one host, especially at high `-w`/`-C` thread counts | Confirmed with `ss -tn` (agent control-connection `Send-Q` non-zero at the exact moment leases expired): outbound traffic volume scales with `-w`/`-C` and can back up the agent's TCP send buffer badly enough to delay the heartbeat behind it. Three fixes have landed (see `docs/DESIGN-agent.md` §3.1–§3.4 for the full history and current status): §3.1 (copy-pool reserve scaling — turned out not to be the cause), §3.2 (dedicated writer thread — insufficient alone), §3.3 (priority mailbox — cut one reporting fleet's rate from 8.5%→1.68%, peak Send-Q 1657→low 300s: a large improvement, not full resolution). §3.4 is open: reproduced on a **single active agent** at the same thread counts (ruling out cross-agent coordinator contention specifically, though not the coordinator's single store-write lock being held by its own background jobs — the lease sweeper, Shard Reaper, pass-phase advance, journal flusher — which is process-wide and shared by even one agent's dispatch). Diagnostic logging on both sides (`"heartbeat queued/sent"`, `"heartbeat received"`, `"dispatch took unusually long"`, `"store: long wait for write lock"` with a `caller` label, `"control loop poll stall"`) is what will pin down the actual mechanism next — check those lines around a requeue event. |
