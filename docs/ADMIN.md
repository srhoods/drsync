...
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine, especially at high `-w` (walker) thread counts | Root cause: the agent's held-lease table (guarded by one mutex, taken by every shard grant/start/complete *and* by the heartbeat's own snapshot) scanned its entire lifetime history on every operation, not just currently-held leases — a high-water mark that only ever grew. At 34+ walker threads churning fast shards (a job with little/nothing to copy turns walk shards over quickly), that scan's cost climbed over a job's lifetime and contended badly enough, under one lock, to starve the heartbeat with **zero footprint on the network or coordinator side** — reproduced on a single active agent with `-S` (work-stealing off) and `-C` cranked high with no effect, isolating it to walker thread count alone. Fixed in `state.c`: the lease table is now O(1)-per-operation (intrusive free list + active list + hash-by-id), not O(lifetime lease count). See `docs/DESIGN-agent.md` §3.1–§3.5 for the full investigation, including two earlier fixes (§3.1 copy-pool reserve scaling, §3.2 dedicated writer thread, §3.3 priority mailbox) that each helped a related but different symptom without being the root cause, and the negative instrumentation results (§3.4: neither coordinator dispatch timing nor store-lock wait time ever showed a delay) that ruled out the coordinator side and pointed back at the agent. |
