...
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine, and it happens on **every** agent, not one host, especially at high `-w`/`-C` thread counts | Root-caused to the agent's control thread blocking on a full TCP send buffer: outbound frames (results, journal batches — volume scales with thread count) shared one blocking socket write path with the heartbeat, so a backed-up send buffer (confirmed with `ss -tn`: agent control-connection `Send-Q` non-zero at the exact moment leases expired) silently stalled heartbeats for as long as the buffer stayed full, expiring every held lease at once. Fixed by moving outbound writes to a dedicated writer thread so a full send buffer can no longer delay the heartbeat timer — see `docs/DESIGN-agent.md` §3.2. (An earlier fix in the same investigation, §3.1's copy-pool reserve scaling, targeted work-stealing contention and turned out not to be the cause — a `-w 14 -C 42` fleet with that fix applied still saw the requeue rate, which is what pointed at the send buffer instead.) To confirm this is still your cause on an up-to-date build: `ss -tn \| grep :<agent-port>` on an agent during a run and watch `Send-Q` on the coordinator connection. |
