...
- **Hardlinks are preserved by default**: hardlink-group members are linked to a
  shared destination copy (`docs/DESIGN-hardlinks.md`); see below. Set
  `metadata.hardlinks: report` to opt out (D3 behavior — nlink>1 files copied as
  independent files, with the duplicated bytes reported per pass).
