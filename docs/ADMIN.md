...
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine, and it happens on **every** agent, not one host, especially at high `-w`/`-C` thread counts | Confirmed with `ss -tn` (agent control-connection `Send-Q` non-zero at the exact moment leases expired): outbound traffic volume (results, journal batches — scales with `-w`/`-C`) can back up the agent's TCP send buffer badly enough to delay the heartbeat behind it. Three fixes have landed, each addressing a real but incomplete part of the mechanism — see `docs/DESIGN-agent.md` §3.1–§3.4 for the full history: §3.1 (copy-pool reserve scaling — targeted work-stealing, turned out not to be the cause), §3.2 (dedicated writer thread — insufficient alone, a heartbeat could still queue behind a large already-queued batch), §3.3 (priority mailbox — cut a reporting fleet's rate from 8.5%→1.68% and peak Send-Q from 1657→low 300s, a large improvement but not full resolution). §3.4 (open, under investigation as of the last update to this table): a residual pattern remains — Send-Q pulses, then a requeue fires roughly one lease TTL later — with diagnostic logging added on both agent and coordinator (`"heartbeat queued/sent"`, `"heartbeat received"`, `"dispatch took unusually long"`, `"control loop poll stall"`) to distinguish an agent-side single-large-write delay from coordinator-side store-mutex contention. If you're chasing this: check those log lines around a requeue event before assuming either cause. |
| A noticeable share of shards requeue via lease expiry (WebUI queue view's "retry pressure" tile, or `drsync_lease_expiries_total` / `drsync_work_grants_total`) even though jobs otherwise finish fine | A single lease expiry is often nothing — a heartbeat frame or `ShardResult` lost once — but a *sustained percentage* points at something worth chasing. Check `drsync_lease_expiries_by_agent_total{agent,kind,outcome}` (Prometheus) or grep the coordinator log for `"lease expired"` (one line per expired shard, carrying `job`/`pass`/`shard`/`kind`/`path`/`agent`/`attempt`/`outcome`): if expiries cluster on one `agent` label, that host is flaky (network, CPU/GC stall under an oversubscribed `-w`/`-C`, or connection churn — cross-check `"agent connected"`/`"agent disconnected"` log lines and `drsync_agent_up{agent}` for that host). If they're spread evenly across every agent instead, suspect something systemic: the coordinator itself stalling under write-lock contention (a large grant or reap batch delaying heartbeat processing past the TTL for several agents at once), a shared network segment, or `-lease-ttl` tuned too tight for real-world heartbeat jitter. See `docs/DESIGN-coordinator.md` §7. |
