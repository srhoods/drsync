...
| 7 | `ShardSplit` | A→C | new shards discovered mid-walk (subdirectories pushed back, or entry-list batches from a huge directory), or delete-remainder batches from a pathological orphan directory being fanned out during a DELETE pass (`DeleteRemainder`, docs/DESIGN-coordinator.md §2.2); also carries `LinkSighting`s (D11, nlink>1 files — sent unconditionally, acted on unless the job opted out via `hardlinks: report`); coordinator persists + queues them, acks with assigned shard ids |
...
message LinkTask {             // D11, default behavior: linkat a hardlink-group
  ...
  uint64 anchor_gen = 4;       // anchor's (mtime,size) snapshot; aborts if the anchor drifted
}
```
