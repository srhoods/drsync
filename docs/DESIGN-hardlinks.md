...
registry is disposable scratch), but real, repeated per-pass work proportional to
hardlink density, not proportional to job length.

Reaped per-pass, not deferred to job purge: `store.ReapLinkRegistry`, called from
`passctrl.advance()` right after the DIRFIX→LINKFIX transition — see §3.6 for the
full argument and why that point is safe. This was originally shipped as job-purge-only
(the `chunk_groups` lifecycle), a simplification made once the real `chunk_groups`
precedent turned out not to per-pass-reap either at the time — closed after a live
400M-file job accumulated 10M `link_groups` / 18.5M `link_members` rows under that
lifecycle.
...
**per pass** — reaped at that pass's own DIRFIX→LINKFIX transition (§3.5), not
accumulated across the job's passes until purge. This is meaningfully larger than the
...
- **Reaped per-pass, not deferred to job purge** (as of the fix chasing a live report:
  10M `link_groups` / 18.5M `link_members` rows on a 400M-file job, generated by the
  previous release before this reap existed). `store.ReapLinkRegistry`, called from
  `passctrl.advance()` right after the DIRFIX→LINKFIX `SetPassState` commits — the
  same tick as `reapPhase(KindDirfix)` there — deletes the pass's `link_groups` rows
  (and every `link_members` row belonging to them, via the `link_members_group` index)
  in `LinkRegistryReapBatchSize`-row batches, mirroring the Shard Reaper's design
  (`DESIGN-coordinator.md` §3) rather than `chunk_groups`'s job-purge-only lifecycle.
  Safe because `passctrl.seedLinkfix` — called immediately before, in the same
  transition — is the *only* reader of either table anywhere in the coordinator, and
  the protocol's own split-before-result ordering invariant (§3.1/§3.2: every
  `ShardSplit`'s sightings are recorded via `RecordSplit` before that shard's own
  `ShardResult` is sent) means no sighting for this pass can arrive after SCANNING is
  proven drained — which happens well before DIRFIX even starts, let alone LINKFIX. So
  by the time `seedLinkfix` has run, a pass's registry rows are permanently settled:
  nothing will read or write them again. See `store.ReapLinkRegistry`'s doc comment for
  the full argument, and `TestReapLinkRegistryDeletesBothTables`/
  `TestReapLinkRegistryBatches`/`TestAdvanceReapsLinkRegistryOnLinkfixTransition`
  (`coordinator/internal/store`, `coordinator/internal/passctrl`) for the regression
  pins.
